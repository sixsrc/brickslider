...
export class BaseSlider {
  ...
  /*  private setTargetSlides(): void {
    ...
    const { infinite, currentSlideMovement: mov, numberOfPages } = this.store
  ...
  private setTargetSlides(): void {
    ...
    // Corrigir o índice inicial no modo infinito
    let leftClones = 0
    if (infinite) {
